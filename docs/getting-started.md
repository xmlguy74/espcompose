...
`useScript()` integrates naturally with the `defineProject()` authoring model.
All hooks must be called inside a component function body:
...
`useScript()` returns a `ScriptHandle` that can be called from trigger
